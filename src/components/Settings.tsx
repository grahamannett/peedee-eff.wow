...
export function Settings({ onBack }: SettingsProps) {
	...
	const [modelsDownloaded, setModelsDownloaded] = useState<boolean | null>(
		null,
	);
	...
	return (
		<div className="min-h-screen flex flex-col bg-bg">
			...
			<header className="bevel-raised bg-bg-secondary flex items-center gap-2 px-3 py-1">
				...
				{saved && (
					<span className="text-xs text-success ml-auto font-bold">Saved</span>
				)}
			</header>
			...
			<main className="flex-1 overflow-y-auto p-3 max-w-md mx-auto w-full space-y-3">
							...
							>
								<input
									...
									onChange={() => {
										...
										if (formats.length > 0) {
											update({
												defaultOptions: { ...opts, output_formats: formats },
											});
										}
									}}
								/>
					...
					<div className="space-y-2">
						<label className="flex items-center gap-1 cursor-pointer text-xs">
							<input
								...
								onChange={(e) =>
									update({
										defaultOptions: { ...opts, force_ocr: e.target.checked },
									})
								}
							/>
							Force OCR
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									update({
										defaultOptions: { ...opts, language: e.target.value },
									})
								}
				...
				<fieldset className="group-box">
					...
					<div className="space-y-1">
						...
						<p className="text-xs text-text-muted">Tauri + React + Marker</p>
					</div>
				</fieldset>
			</main>
		</div>
